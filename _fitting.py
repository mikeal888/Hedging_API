...
def stock_pdf(s, S0, t, mu, sigma):
    ...
    pdf = (1 / (s * sigma * np.sqrt(2 * np.pi * t))) * np.exp(
        -((np.log(s / S0) - (mu - 0.5 * sigma**2) * t) ** 2) / (2 * sigma**2 * t)
    )
...
def fit_geometric_brownian_motion(data, params_guess, dt):
    ...
    with pm.Model() as stock_model:
        ...
        stock = EulerMaruyama(
            "stock", dt, lin_sde, (mu, sigma), shape=len(data), initval=data
        )
...
def log_normal_pdf(data):
    """
    Create a log normal probability density function using stats.lognorm.pdf
    """
